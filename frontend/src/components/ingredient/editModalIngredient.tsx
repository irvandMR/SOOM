...
export default function EditModalIngredient({
    ...
    // Load ingredient data saat modal dibuka
    useEffect(() => {
    ...
    // hanya unit yang bisa convert yang editable
    const isUnitEditable = (symbol: string) =>
      ['kg', 'g', 'mg', 'L', 'mL'].includes(symbol)

    const handleUnitChange = (newUnitId: string) => {
      const newUnit = units.find(u => u.id === newUnitId)
      if (!newUnit) return

      // hanya convert jika bisa
      if (canConvertUnit(unitSymbol, newUnit.symbol)) {
        setForm(prev => ({
          ...prev,
          stockQuantity: convertUnitQuantity(unitSymbol, newUnit.symbol, prev.stockQuantity),
          minimumStock: convertUnitQuantity(unitSymbol, newUnit.symbol, prev.minimumStock),
          unitId: newUnitId
        }))
      }
      ...
      >
        ...
        <FormField label="Unit" required>
          <Dropdown
            ...
            disabled={!isUnitEditable(unitSymbol)}
          />
          {errors.unitId && <small className="p-error">{errors.unitId}</small>}
        </FormField>

        {/* Stok & Minimum Stock */}
        <FormField label={`Stok (${unitSymbol})`} required>
          <InputNumber
        ...
        <FormField label={`Minimum Stok (${unitSymbol})`} required>
          <InputNumber
        ...
        {/* Harga Beli */}
        <FormField label={`Harga Beli (${unitSymbol})`} required>
          <InputNumber
